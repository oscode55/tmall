...
import com.lijiecheng.tmall.common.TokenCache;
import com.lijiecheng.tmall.dao.UserMapper;
import com.lijiecheng.tmall.pojo.User;
import com.lijiecheng.tmall.service.IUserService;
import com.lijiecheng.tmall.util.MD5Util;
...
//        //填充自己的sql查询逻辑
        ...
        pageResult.setList(assemble(userList));
        return ServerResponse.createBySuccess(pageResult);
    }
    private List<User> assemble(List<User> userList){
    ...
    public ServerResponse<String> register(User user) {
        ...
        if (!validResponse.isSuccess()) { //用户名验证是否已存在
            return validResponse;
        }
        validResponse = this.checkValid(user.getEmail(), Const.EMAIL);
        if (!validResponse.isSuccess()) { //邮箱是否已存在
            return validResponse;
        }
        user.setRole(Const.Role.ROLE_CUSTOMER); //设置身份为普通会员
        //MD5加密
